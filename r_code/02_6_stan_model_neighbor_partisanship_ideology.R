#!/usr/bin/Rscript
#SBATCH --job-name=mcmc_ratio_partisanship_ideology
...
Z <- model.matrix(~ std_age + gender + educ + d_hispanic + d_midwest + d_south + d_west	+ d_black + d_asian + d_female + d_dem + d_rep + d_party_other + d_liberal + d_conservative  , data=demographic_data)

save(X,Z, file="../data/stan_draws/ratio_partisanship_ideology_supp.Rda")
...
fit$save_object("../data/stan_draws/neighbor_partisanship_ideology_draws.Rda")
